...
import { setManagerActive } from "@/lib/actions/managers";
import { Power, Loader2 } from "lucide-react";

export function ManagerActiveToggle({
  managerId, isActive,
}: {
  managerId: string;
  isActive: boolean;
  ...
  function handle() {
    if (!confirm(
      isActive
        ? "Deactivate this manager? They won't be able to sign in until reactivated."
        : "Reactivate this manager?"
    )) return;
    setError(null);
    startTransition(async () => {
      const res = await setManagerActive(managerId, !isActive);
