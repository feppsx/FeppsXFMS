...
export default async function ManagerFeedbackList({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  return (
    <>
      <MobileHeader title="Customer Feedback" showBack backHref="/technician/jobs" />
      ...
      <form method="GET" className="bg-white border border-slate-200 rounded-2xl p-4 shadow-card mb-4 grid md:grid-cols-5 gap-3 text-sm">
        ...
        <div className="md:col-span-5 flex gap-2">
          ...
          <Link href="/technician/feedback" className="border border-slate-300 rounded-lg px-4 py-1.5 text-sm font-medium hover:bg-slate-50">Clear</Link>
        </div>
      </form>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="p-3">
                    {r.ticket ? (
                      <Link href={`/technician/jobs/${r.ticket.id}`} className="text-brand hover:underline">
